...
#include "bpf/small_incremental_read_only_ks_bpf.bin.h"
...
void launch_test_case(KS_Test_State* ks_state){
	bpf_t ks_bpf = {
	    .application = small_incremental_read_only_ks_bpf_bin,
    	.application_len = sizeof(small_incremental_read_only_ks_bpf_bin),
        .stack = _stack,
	...
	print_csv_body(test_runs, runs, "Small Incremental RO FC in ms,");
	...
	print_csv_body(test_runs, runs, "Small Incremental RO Plain in ms,");
}
