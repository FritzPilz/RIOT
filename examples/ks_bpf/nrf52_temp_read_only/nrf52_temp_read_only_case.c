...
void launch_test_case(KS_Test_State* ks_state){
	bpf_t ks_bpf = {
	    .application = nrf52_temp_read_only_bpf_bin,
    	.application_len = sizeof(nrf52_temp_read_only_bpf_bin),
        .stack = _stack,
	...
	print_csv_body(test_runs, runs, "Temperature RO FC in ms,");
	...
	print_csv_body(test_runs, runs, "Temperature RO Plain in ms,");
}
...
void run_bpf_test(bpf_t* ks_bpf, kolmogorov_ctx_t* ctx, benchmark_runs* test){
	...
	for(uint32_t i = 0; i < test->times_to_run; ++i){
		uint32_t value = bpf_execute_ctx(ks_bpf, ctx, sizeof(*ctx), &res);
