...
#include "small_incremental_read_only_ks.wasm.h"
...
static wasm_module_t small_incremental_read_only_module;
static wasm_module_inst_t small_incremental_read_only_instance = NULL;
...
void launch_test_case(KS_Test_State* ks_state){	
	...
	print_csv_body(test_runs, runs, "Small Incremental RO WASM in ms,");
...
int32_t prepare_wasm_run(uint8_t* wasm_buf){
	...
	memcpy(wasm_buf, small_incremental_read_only_ks_wasm, sizeof(small_incremental_read_only_ks_wasm));

	char error_buf[128];
    if (!(small_incremental_read_only_module = wasm_runtime_load(wasm_buf, sizeof(small_incremental_read_only_ks_wasm),
        error_buf, sizeof(error_buf)))) {
        puts(error_buf);
		return -1;
    }

    if (!(small_incremental_read_only_instance = wasm_runtime_instantiate(small_incremental_read_only_module, 8 * 1024,
...
void cleanup_wasm(uint8_t* wasm_buf){
	wasm_runtime_deinstantiate(small_incremental_read_only_instance);
    free(wasm_buf);
}