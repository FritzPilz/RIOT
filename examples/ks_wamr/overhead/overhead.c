...
void launch_test_case(KS_Test_State* ks_state){	
	...
	xtimer_sleep(1);
	print_csv_body(test_runs, runs, "Overhead WASM in ms,");
	...
	xtimer_sleep(1);
	print_csv_body(test_runs, runs, "Overhead Plain in ms,");
}
...
int32_t prepare_wasm_run(uint8_t* wasm_buf){
	...
	wasm_buf = malloc(sizeof(overhead_wasm));
	if(!wasm_buf){
		return -1;
	}
	memcpy(wasm_buf, overhead_wasm, sizeof(overhead_wasm));

	char error_buf[128];
    if (!(incremental_module = wasm_runtime_load(wasm_buf, sizeof(overhead_wasm),
